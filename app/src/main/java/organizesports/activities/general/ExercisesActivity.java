...
public class ExercisesActivity extends AppCompatActivity implements DataAdapter.DataListener {
    ...
    private void getData() {
        ...
        new DataController().getExercisesAlways(new DataCallback() {
            @Override
            public void onSuccess(ArrayList<DataModel> exerciseTypes) {
                ...
                if (currentList.size() > 0) {
                    ...
                    if (adapter == null || adapter.getData().size() == 0) {
                        adapter = new DataAdapter(currentList, ExercisesActivity.this);
                        recyclerView.setAdapter(adapter);
                        recyclerView.setLayoutManager(new LinearLayoutManager(ExercisesActivity.this));
                    } else {
            ...
            public void onFail(String error) {
                loadingHelper.dismissLoading();
                Toast.makeText(ExercisesActivity.this, error, Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    public void deleteItem(int position) {
        new DataController().delete(adapter.getData().get(position), new DataCallback() {
            @Override
            public void onSuccess(ArrayList<DataModel> exerciseTypes) {
                Toast.makeText(ExercisesActivity.this, "Deleted!", Toast.LENGTH_LONG).show();
            }
